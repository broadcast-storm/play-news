...
export const doCreateUserWithEmailAndPassword = (login, email, password) => {
  return async (dispatch, getState) => {
    const { firebase } = getState();
    try {
      const authNew = firebase.auth;
      const authUser = await authNew.createUserWithEmailAndPassword(
        email,
        password
      );
      ...
      const newDb = firebase.db;
      await newDb.ref(`users/${authUser.user.uid}`).set({
        email: email,
        login: login
      });
...
export function deleteRemind(id) {
  return async (dispatch, getState) => {
    try {
      ...
      const newDb = firebase.db;
      await newDb
        .ref("/users/" + firebase.authUser.uid + "/reminds/" + id)
        .remove();
...
export function deleteNote(id) {
  return async (dispatch, getState) => {
    try {
      ...
      const newDb = firebase.db;
      await newDb
        .ref("/users/" + firebase.authUser.uid + "/notes/" + id)
        .remove();
